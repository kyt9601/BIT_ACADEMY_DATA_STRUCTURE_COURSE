...
typedef int (*FP);   // FP 작성
typedef int (*FPP);   // FPP 작성
...
____ calculator3 ____ {
      int low = 0;
      int high = arr_size-1; 
      int random_Num = rand()%(high-low+1) + low; //난수 발생범위를 low에서 high 범위내에 출력되는 코드
...
int calculator2 (____){
    ...
    random_Num = rand()%(high-low+1) + low; //난수 발생범위를 low에서 high 범위내에 출력되는 코드
    printf("난수 : %d\n", random_Num);
